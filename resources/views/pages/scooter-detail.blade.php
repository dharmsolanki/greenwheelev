        ...
        <div class="section-inner">
            <div class="grid-2" style="gap:40px;align-items:start">
                ...
                <div>
                    @if ($scooter->images->count() > 0)
                        <div class="scooter-slider">
                            ...
                            <div class="slider-main">
                                <img id="mainImg" src="{{ asset('storage/' . $scooter->images->first()->image_path) }}"
                                    alt="{{ $scooter->name }}"
                    ...
                    @else
                        <div
                            style="background:var(--green3);border-radius:16px;padding:80px;text-align:center;font-size:100px">
                            {{ $scooter->icon }}</div>
                    @endif
                </div>
                ...
                <div>
                    ...
                    @if ($scooter->tag)
                        <span class="tag-badge" style="margin-bottom:16px;display:inline-block">{{ $scooter->tag }}</span>
                    @endif
                    <div style="font-size:30px;font-weight:800;color:var(--green);margin:16px 0 24px">
                        ₹{{ number_format($scooter->price) }}</div>

                    {{-- Specs --}}
                    <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:12px;margin-bottom:24px">
                        @foreach ([['bolt', 'Range', $scooter->range], ['tachometer-alt', 'Top Speed', $scooter->top_speed], ['plug', 'Charging', $scooter->charging_time], ['cog', 'Motor', $scooter->motor_power]] as [$ico, $lbl, $val])
                            <div
                                style="background:var(--green3);border-radius:10px;padding:14px;display:flex;gap:10px;align-items:center">
                                <span style="font-size:22px"><i class="fas fa-{{ $ico }}"></i></span>
                                <div>
                                    <div style="font-size:15px;font-weight:700;color:var(--green-dark)">{{ $val }}
                                    </div>
                                    <div style="font-size:11px;color:#338a57">{{ $lbl }}</div>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    @if ($scooter->description)
                        <p style="color:#555;line-height:1.8;margin-bottom:20px;font-size:14.5px">
                            {{ $scooter->description }}</p>
                    @endif

                    <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:20px">
                        <a href="{{ route('contact.index') }}" class="btn-primary"><i class="fas fa-phone-alt"></i> Book
                            Now</a>
                        <a href="https://wa.me/917984304504?text=Hi!+I+am+interested+in+{{ urlencode($scooter->name) }}"
                            target="_blank" class="btn-outline" style="color:var(--text);border-color:var(--border)"><i
                                class="fab fa-whatsapp" style="color:#25d366"></i> WhatsApp</a>
                        <a href="#test-ride" class="btn-outline" style="color:var(--text);border-color:var(--border)"><i
                                class="fas fa-route"></i> Test Ride</a>
                    </div>
                    ...
                    <div class="grid-3">
                        @foreach ($related as $sc)
                            <div class="scooter-card">
                                @if ($sc->images->first())
                                    <img src="{{ asset('storage/' . $sc->images->first()->image_path) }}"
                                        alt="{{ $sc->name }}" style="width:100%;height:180px;object-fit:cover">
                                @else
                ...
                <div class="form-card" style="max-width:540px;margin:0 auto">
                    ...
                    <form method="POST" action="{{ route('test-ride.book') }}">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group full"><label>Preferred Date *</label><input type="date"
                                    name="preferred_date" required min="{{ date('Y-m-d', strtotime('+1 day')) }}"></div>
                        </div>
