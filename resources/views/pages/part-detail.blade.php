    ...
    <div class="grid-2">
      ...
      <div>
        ...
        <div style="margin-bottom:16px">
          @if($part->stock > 10)<span style="color:var(--green);font-weight:600">✅ In Stock ({{ $part->stock }} units)</span>
          @elseif($part->stock > 0)<span style="color:#f59e0b;font-weight:600">⚠️ Only {{ $part->stock }} left</span>
          @else<span style="color:#ef4444;font-weight:600">❌ Out of Stock</span>@endif
        </div>
        @if($part->description)<p style="color:#555;line-height:1.8;margin-bottom:20px">{{ $part->description }}</p>@endif
        <div style="display:flex;gap:10px;flex-wrap:wrap">
          @if($part->stock > 0)<button onclick="addToCart({{ $part->id }})" class="btn-primary">🛒 Add to Cart</button>@endif
