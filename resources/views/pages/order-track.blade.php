    ...
    <div class="admin-card">
      ...
      <div style="display:flex;justify-content:space-between;padding:20px 0;position:relative">
        ...
        <div style="display:flex;flex-direction:column;align-items:center;position:relative;z-index:1">
          <div style="width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:14px;background:{{ $i<=$ci?'var(--green)':'#e0e0e0' }};color:{{ $i<=$ci?'white':'#999' }}">{{ $i<=$ci?'✓':$i+1 }}</div>
